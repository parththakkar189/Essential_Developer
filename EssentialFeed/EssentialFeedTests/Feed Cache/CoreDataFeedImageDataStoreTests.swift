...
final class CoreDataFeedImageDataStoreTests: XCTestCase {
    ...
    func test_retriveImageData_deliversFounDataWhenThereIsAStoredImageDataMatchingURL() {
        let sut = makeSUT()
    ...
    func test_sideEffects_runSerially() {
        ...
        let op2 = expectation(description: "Opertation 2")
        sut.insert([localImage(url: url)], timeStamp: Date()) { _ in
            op2.fulfill()
        }
        
        let op3 = expectation(description: "Opertation 3")
        sut.insert([localImage(url: url)], timeStamp: Date()) { _ in
            op3.fulfill()
        }
